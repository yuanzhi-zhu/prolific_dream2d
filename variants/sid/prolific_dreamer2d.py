...
def main():
    ...
    ### sds text to image generation
        ...
        for step, chosen_t in enumerate(pbar):
            ...
            # loss step
            noise_pred, noise_pred_phi = sid_grad_diffuser(unet, noisy_latents, noise, text_embeddings_vsd, t, \
            ...
            pred_x0_fake = scheduler.step(noise_pred_phi, t, noisy_latents).pred_original_sample
            pred_x0_true = scheduler.step(noise_pred, t, noisy_latents).pred_original_sample
            with torch.no_grad():
                weight_factor = abs(latents_vsd.to(torch.float32) - pred_x0_true.to(torch.float32)).mean(dim=[1, 2, 3], keepdim=True).clip(min=0.00001)
            loss = (pred_x0_true - pred_x0_fake) * (pred_x0_fake - latents_vsd) / weight_factor
